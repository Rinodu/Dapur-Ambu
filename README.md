...
Form di `order.html` menyiapkan ringkasan pesanan untuk dikirim pelanggan lewat WhatsApp ke nomor Dapur Ambu. Data tidak tersimpan otomatis di situs; formulir Google Apps Script lama tetap ditautkan sebagai alternatif. Jalankan `node test-order.mjs` untuk memeriksa tautan WhatsApp dan `node test-intro.mjs` untuk memeriksa alur kembali ke beranda.
